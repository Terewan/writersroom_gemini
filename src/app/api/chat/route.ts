    ...
    try {
        ...
        switch (provider) {
            case 'anthropic':
                ...
                break
            case 'google':
                const googleKey = userKeys?.gemini || process.env.GOOGLE_GENERATIVE_AI_API_KEY
                if (!googleKey) throw new Error("Google Gemini API Key is missing. Please add it in Settings.")
                const customGoogle = createGoogleGenerativeAI({ apiKey: googleKey })
                // Gemini requires the 'models/' prefix for the rest API
                const finalModel = model.startsWith('models/') ? model : `models/${model}`
                aiModel = customGoogle(finalModel)
                break
            case 'openai':
            default:
        ...
        return new Response(JSON.stringify({ error: e.message || 'Unknown error occurred in AI chat route.' }), { status: 500 })
    }
}
